 ...
 * 			 -3 there was an error creating the socket to connect with the local server
 * 			 -4 there was an error connecting to the local server
 * 			 -5 the local server didn't respond on a set amount of time
 * 			 -6 the server didn't respond at all
 *
 */
 ...
 * 			 -2 there was an error sending the data to the socket
 * 			 -3 there was an error recieving the package from the local server
 * 			 -4 the group doesnt exist. Must close the connection
 * 			 -5 there was an error sending the size of the value to the socket
 * 			 -6 there was an error sending the value to the socket
 */
int put_value(char *key, char *value) {
	...
	size = strlen(value) + 1;
	if (send(local_server_sock, (void *)&size, sizeof(size_t), 0) < 0) {
		perror("sending size of value");
		return -5;
	}
 ...
 * 			 -2 there was an error sending the package to the socket
 * 			 -3 there was an error recieving the package from the local server
 * 			 -4 the group doesn't exist. Must close the connection
 *
 */
 ...
 *   callback_function(char*): pointer to function
 *
 *   returns: 0 the callback was successfully implemented
 *			 -1 the function couldnt allocate memory for the callback data
 *
 */
 ...
 *			 -1 couldn't send data to the local server
 *			 -2 couldn't close the socket connecting to the local server
 *
 */
